...
export function getProjectEnvironmentFile(project: WorkspaceProject): string {
  ...
  if (
    ...
    throw new SchematicsException(`Could not find the configuration of the workspace config (${project.sourceRoot})`);
  }

  const fileReplacements: [{ replace: string; with: string }] = configurations.production.fileReplacements;
